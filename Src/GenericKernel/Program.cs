...
{
    /// <summary>
    /// An interface constraint for the <see cref="Kernel{TKernelFunction, T}(Index, ArrayView{T}, int, TKernelFunction)"/> function.
    /// This helps to emulate a lambda-function delegate that is passed to a kernel in a type safe way.
    /// </summary>
    /// <typeparam name="T">The element type that is returned by the <see cref="ComputeValue(Index, int)"/> function.</typeparam>
        ...
        where T : struct
    {
        ...
        T ComputeValue(Index index, int value);
    }
    ...
    readonly struct LambdaClosure : IKernelFunction<long>
    {
        ...
        /// <summary cref="IKernelFunction{T}.ComputeValue(Index, int)"/>
        public long ComputeValue(Index index, int value) =>
            Offset + value * index;
    }
                ...
                foreach (var acceleratorId in Accelerator.Accelerators)
                {
                    ...
                    using (var accelerator = Accelerator.Create(context, acceleratorId))
                    {
                        ...
                        var kernel = accelerator.LoadAutoGroupedStreamKernel<
                            Index, ArrayView<long>, int, LambdaClosure>(Kernel);
