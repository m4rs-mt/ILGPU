...
partial class Method
{
        ...
        IDumpable
    {
        ...
        private bool _updateControlFlow;
        private bool _acceptControlFlowUpdates;

        /// <summary>
        /// Indicates if the builder completed successfully. If not, skip some of the
        /// processing that occurs during disposal.
        /// </summary>
        private bool _builderCompleted;

        /// <summary>
        /// All created basic block builders.
        /// </summary>
        private readonly List<BasicBlock.Builder> _basicBlockBuilders = [];
        ...
        {
            Method = method;
            _parameters = method.parameters.ToBuilder();
            _blockCounter = method.Blocks.Count;
        ...
        public BasicBlock.Builder this[BasicBlock basicBlock]
        {
            get
            {
                ...
                {
                    _basicBlockBuilders.Add(basicBlockBuilder);
                }
        ...
        /// </summary>
        public int NumParams => _parameters.Count;
        ...
        /// </summary>
        public void ScheduleControlFlowUpdate() => _updateControlFlow = true;
        ...
        public void AcceptControlFlowUpdates(bool accept) =>
            _acceptControlFlowUpdates = accept;
        ...
        public void AssertNoControlFlowUpdate() =>
            Method.Assert(!_updateControlFlow | _acceptControlFlowUpdates);
        ...
        public BasicBlock.Builder CreateBasicBlock(Location location, string? name)
        {
            ...
            block.BeginControlFlowUpdate(_blockCounter++);
            ScheduleControlFlowUpdate();
            return this[block];
        }
            ...
            where TOrder : struct, ITraversalOrder
        {
            ...
            var newBlocks = ImmutableArray.CreateBuilder<BasicBlock>(_blockCounter);
        ...
        {
            Debug.Assert(!_builderCompleted);
            _builderCompleted = true;
        }
        ...
        private void UpdateParameters()
        {
            ...
            for (int i = 0; i < _parameters.Count;)
            {
                if (_parameters[i].IsReplaced)
                    _parameters.RemoveAt(i);
                else
                    ++i;
            }
            var @params = _parameters.ToImmutable();
        ...
        {
            if (!_updateControlFlow)
                return SourceBlocks;
            _updateControlFlow = false;
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (_builderCompleted)
                UpdateParameters();

            // Dispose all basic block builders
            foreach (var builder in _basicBlockBuilders)
                builder.Dispose();
