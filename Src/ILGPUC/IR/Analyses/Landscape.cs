...
class Landscape<T>
{
    ...
    internal sealed class Entry
    {
        ...
        private readonly HashSet<Method> _usesSet = [];
        private List<Method>? _uses;
        ...
        /// </summary>
        public int NumUses => _usesSet.Count;
        ...
        public bool IsUsedBy(Method method) => _usesSet.Contains(method);

        /// <summary>
        /// Registers the given method use.
        /// </summary>
        /// <param name="method">The method to register.</param>
        internal void AddUse(Method method) => _usesSet.Add(method);
    ...
    /// </summary>
    internal struct Enumerator : IEnumerator<Entry>
    {
        private int _index;
        ...
        /// </summary>
        public readonly Entry Current => Landscape._postOrder[_index];
    ...
    private readonly Dictionary<Method, Entry> _entries = [];
    private readonly List<Entry> _sinks = [];
    private readonly List<Entry> _postOrder = [];
    ...
    /// </summary>
    public int Count => _postOrder.Count;
    ...
    public bool TryGetEntry(Method method, [NotNullWhen(true)] out Entry? entry) =>
        _entries.TryGetValue(method, out entry);
        ...
        where TDataProvider : IDataProvider
    {
        ...
        var methodSet = methods.ToSet();

        // Iterate over all methods in the entry set and resolve their references
        foreach (var method in methods)
        {
            var references = References.Create(method, methodSet.Contains);
        ...
        foreach (var entry in _entries.Values)
        {
            ...
            {
                if (_entries.TryGetValue(reference, out Entry? referenceEntry))
                    referenceEntry.AddUse(entry.Method);
            }
        }

        Parallel.ForEach(_entries.Values, entry => entry.FinishUses());

        if (_sinks.Count > 0)
        {
            // Sort sinks to have a deterministic order
            _sinks.Sort(Entry.Comparison);
            ComputeOrder();
        }
    }
    ...
    private void ComputeOrder()
    {
        var toProcess = new Stack<(Entry, List<Method>.Enumerator, bool)>(
            _entries.Count - _sinks.Count);
        var visited = new HashSet<Entry>();
        var currentSink = 0;
        var current = (
            Fun: _sinks[0],
            Enumerator: _sinks[0].GetEnumerator(),
            Init: true);
        ...
        next:
            ...
            {
                if (++currentSink >= _sinks.Count)
                    break;

                // Check next sink
                var nextSink = _sinks[currentSink];
...
/// This includes the call graph, function size and dependency information.
/// </summary>
sealed class Landscape : Landscape<object>
{
    #region Nested Types
