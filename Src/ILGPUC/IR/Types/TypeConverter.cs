...
abstract class TypeConverter<TType> : ITypeConverter<TypeNode>
    where TType : TypeNode
{
    /// <summary>
    ...
    public TypeNode ConvertType<TTypeContext>(
        ...
        type switch
        {
            TType typeNode => ConvertType(typeContext, typeNode),
    ...
    public int GetNumFields(TypeNode type) =>
        type is TType typeNode
        ? GetNumFields(typeNode)
        : 1;
}
...
abstract class TypeLowering<TType> : TypeConverter<TType>
    where TType : TypeNode
{
    private readonly Dictionary<Value, TypeNode> typeMapping = new();
