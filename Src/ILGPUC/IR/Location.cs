...
abstract class Location : ILocation
{
    ...
    {
        if (!start.IsKnown) return end;
        if (!end.IsKnown) return start;
...
class FileLocation : Location
{
    ...
    public override int GetHashCode() =>
        HashCode.Combine(StartLine, EndLine, StartColumn);
...
/// </summary>
sealed class CompilationStackLocation : Location
{
    #region Instance
    ...
    /// <param name="other">The other one to merge with.</param>
    /// <returns>The merged location.</returns>
    public CompilationStackLocation Append(Location other) => new(Stack.Push(other));
