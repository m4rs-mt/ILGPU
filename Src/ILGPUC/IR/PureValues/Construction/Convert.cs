...
partial class PureValueBuilder
{
    ...
    public Value? CreateConvertToInt32(Location location, Value? node) =>
    ...
    /// <returns>A node that represents the convert operation.</returns>
    [return: NotNullIfNotNull(nameof(node))]
    public Value? CreateConvertToInt64(Location location, Value? node) =>
    ...
    public Value? CreateConvert(
        ...
        CreateConvert(
            location,
            node,
            ModuleBuilder.GetPrimitiveType(basicValueType));
        ...
        ConvertFlags flags)
    {
        ...
        if (node is null || node.Type.Equals(targetType))
            return node;

        // Check for int to pointer casts
        if (targetType is PointerType pointerType && pointerType.ElementType is VoidType)
        {
        ...
        if (targetPrimitiveType.IsBool)
        {
            return CreateCompare(
                ...
                CreatePrimitiveValue(
                    ...
                    node.BasicValueType,
                    0),
                CompareKind.NotEqual).AsNotNull();
        }
        // Match bool to X
