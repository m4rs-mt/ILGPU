...
/// </summary>
static partial class PTXInstructions
{
    /// <summary>
        ...
        ArithmeticBasicValueType type)
    {
        ...
        if (unorderedFloatComparison)
            return CompareUnorderedFloatOperations[(kind, type)];
        else
            return CompareOperations[(kind, type)];
    }
    ...
    public static string GetArithmeticOperation(
        ...
        bool fastMath)
    {
        if (kind == UnaryArithmeticKind.Tanh &&
        ...
        else if (kind == UnaryArithmeticKind.Tanh &&
        ...
        bool fastMath)
    {
        ...
        var key = (kind, type);
        return fastMath
            ? BinaryArithmeticOperationsFastMath[key]
            : BinaryArithmeticOperations[key];
    }
    ...
    public static string GetArithmeticOperation(
        ...
        ArithmeticBasicValueType type) =>
        TernaryArithmeticOperations[(kind, type)];
    ...
    public static string GetAtomicOperation(GenericAtomicKind kind, bool requireResult) =>
    ...
    public static string GetAddressSpaceCastSuffix(BasicValueType pointerType) =>
        AddressSpaceCastOperationSuffix[
            pointerType == BasicValueType.Int32 ? 0 : 1];
