...
partial class PTXCodeGenerator
{
    ...
    internal struct CommandEmitter(
        ...
        string terminator) : IDisposable
    {
        ...
        private readonly StringBuilder _stringBuilder = target;
        private bool _argMode = false;
        private int _argumentCount = 0;
        private readonly string _argumentSeparator = argSeparator;
        private readonly string _commandTerminator = terminator;
        ...
        public readonly void AppendNonLocalAddressSpace(MemoryAddressSpace addressSpace)
        {
        ...
        public readonly void AppendAddressSpace(MemoryAddressSpace addressSpace)
        {
            switch (addressSpace)
            {
                ...
                case MemoryAddressSpace.Shared:
                    _stringBuilder.Append(".shared");
                    break;
                case MemoryAddressSpace.Local:
                    _stringBuilder.Append(".local");
                    break;
                default:
                    break;
            }
        ...
        private void AppendArgument()
        {
            ...
            if (_argumentCount > 0)
                _stringBuilder.Append(_argumentSeparator);
            ++_argumentCount;
        }
        ...
        public void AppendArgument(HardwareRegister argument)
        {
            AppendArgument();
            _stringBuilder.Append('%');
            _stringBuilder.Append(GetStringRepresentation(argument));
        }
        ...
        public void AppendVectorArgument(PrimitiveRegister[] arguments)
        {
            ...
            bool oldArgMode = _argMode;
            int oldArgCount = _argumentCount;
        ...
        public void AppendConstant(float value)
        {
            ...
            AppendArgument();
            _stringBuilder.Append("0f");
            for (int i = 0; i < 4; ++i)
            {
                var part = (intRef >> 24) & 0xff;
                _stringBuilder.Append(part.ToString("X2"));
                intRef <<= 8;
            }
        }
        ...
        public readonly void AppendOffset(int offset)
        {
            ...
            Debug.Assert(_argMode, "Invalid argument mode");
            _stringBuilder.Append('+');
        ...
        {
            AppendArgument();
            _stringBuilder.Append(valueReference);
        }
        ...
        {
            _stringBuilder.Append(_commandTerminator);
    ...
    internal readonly record struct PredicateConfiguration(
    ...
    /// </summary>
    internal interface IComplexCommandEmitter
    {
        /// <summary>
    ...
    /// Encapsulates a complex command emission process.
    /// </summary>
    internal interface IComplexCommandEmitterWithOffsets
    {
        /// <summary>
        /// Emits a nested primitive command in the scope of a complex command chain.
        /// </summary>
    ...
    /// Encapsulates a complex command emission process.
    /// </summary>
    internal interface IVectorizedCommandEmitter : IComplexCommandEmitterWithOffsets
    {
        /// <summary>
        /// Emits a nested primitive command in the scope of a complex command chain.
        /// </summary>
    ...
    internal interface IIOEmitter<T>
        where T : struct
    {
        /// <summary>
