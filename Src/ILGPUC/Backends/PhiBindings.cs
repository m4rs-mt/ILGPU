...
readonly struct PhiBindings
{
    ...
    internal readonly struct IntermediatePhiCollection(BlockInfo info)
    {
        /// <summary>
        ...
        internal struct Enumerator(in BlockInfo blockInfo)
        {
            private HashSet<PhiValue>.Enumerator _enumerator =
                blockInfo.IntermediatePhis.GetEnumerator();

            /// <summary cref="IEnumerator{T}.Current"/>
            public PhiValue Current => _enumerator.Current;
        ...
        /// An enumerator to enumerate all entries in this collection.
        /// </returns>
        public Enumerator GetEnumerator() => new(info);
    }
    ...
    /// </remarks>
    /// <param name="info">The phi information per block.</param>
    internal readonly struct PhiBindingCollection(in PhiBindings.BlockInfo info)
    {
        /// <summary>
        /// An enumerator to enumerate all entries in this collection.
        /// </summary>
        ...
        internal struct Enumerator(PhiBindingCollection collection)
        {
            private List<(PhiValue, Value)>.Enumerator _enumerator =
                collection._blockInfo.Bindings.GetEnumerator();

            /// <summary cref="IEnumerator{T}.Current"/>
            public (PhiValue Phi, Value Value) Current => _enumerator.Current;

            /// <summary cref="IEnumerator.MoveNext"/>
            public bool MoveNext() => _enumerator.MoveNext();
        }

        private readonly BlockInfo _blockInfo = info;

        /// <summary>
        /// Returns all intermediate phi values that must be assigned to temporaries.
        /// </summary>
        public IntermediatePhiCollection Intermediates => new(_blockInfo);
        ...
        public bool IsIntermediate(PhiValue phi) =>
            _blockInfo.IntermediatePhis.Contains(phi);
    ...
    internal readonly struct BlockInfo(int capacity)
    {
        ...
        public List<(PhiValue Phi, Value Value)> Bindings { get; } =
            new List<(PhiValue, Value)>(capacity);
    ...
    private readonly BasicBlockMap<BlockInfo> _phiMapping;
    private readonly InlineList<PhiValue> _allPhiValues;
        ...
        ref InlineList<PhiValue> phiValues)
    {
        ...
        _allPhiValues = InlineList<PhiValue>.Empty;
        phiValues.MoveTo(ref _allPhiValues);
    ...
    /// </summary>
    public ReadOnlySpan<PhiValue> PhiValues => _allPhiValues;
    ...
    {
        bindings = default;
        if (!_phiMapping.TryGetValue(block, out var info))
            return false;
