    ...
    where T : class
{
    ...
    private InlineList<T> _cache;
    private InlineList<T> _used;
    ...
    protected ParallelCache(int? initialCapacity = null)
    {
        ...
        _cache = InlineList<T>.Create(capacity);
        _used = InlineList<T>.Create(capacity);
    ...
    /// </summary>
    protected ReadOnlySpan<T> Used => _used;
    ...
    public void FinishProcessing()
    {
        ...
        _cache.AddRange(_used.AsReadOnlySpan());
        _used.Clear();
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        if (_cache.Count > 0 && typeof(IDisposable).IsAssignableFrom(typeof(T)))
        {
            foreach (var intermediateStates in _cache)
    ...
    where TBody : IParallelProcessingBody<T>
{
    ...
    private readonly Func<int, ParallelLoopState?, T, T> _body;
    private readonly TBody _bodyImplementation;
    private readonly ParallelOptions _defaultOptions = new();
    ...
    {
        _bodyImplementation = CreateBody();
        _body = (i, state, intermediate) =>
        {
            _bodyImplementation.Process(i, state, intermediate);
            return intermediate;
        };
    }
    ...
    public void ParallelFor(int fromInclusive, int toExclusive) =>
        ParallelFor(fromInclusive, toExclusive, _defaultOptions);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void ParallelFor(int fromInclusive, int toExclusive, ParallelOptions options)
    {
        // Initialize processing cache
        InitializeProcessing();

        // Initialize operation
        _bodyImplementation.Initialize();
        ...
        // Finalize operation
        _bodyImplementation.Finalize(Used);
        FinishProcessing();
    }
}
