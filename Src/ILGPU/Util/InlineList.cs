...
public struct InlineList<T>
{
    ...
    public static InlineList<T> Create(int capacity) => new(capacity);
    ...
    {
        _items = capacity < 1 ? [] : new T[capacity];
        Count = 0;
    }
    ...
    /// </summary>
    private void EnsureCapacity(int offset = 1)
    {
    ...
    /// <summary>
    /// Adds the given hash set to this list.
    /// </summary>
    /// <typeparam name="TOther">The element type of the set.</typeparam>
    /// <param name="set">The source set.</param>
    public void AddRange<TOther>(HashSet<TOther> set)
        where TOther : T
    {
        if (set.Count < 1)
            return;
        Reserve(Count + set.Count);
    ...
    public void AddRange<TOther>(ReadOnlySpan<TOther> span) where TOther : T
    {
        if (span.Length < 1)
            return;
    ...
    /// <param name="index">The target index.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Insert(T item1, T item2, int index)
    {
        EnsureCapacity(offset: 2);
        for (int i = Count + 1; i >= index; --i)
            _items[i] = _items[i - 1];
        _items[index + 0] = item1;
        _items[index + 1] = item2;
        Count += 2;
    }
    ...
    public readonly int IndexOf(
        ...
        int defaultValue = -1)
    {
        for (int i = offset, e = Count; i < e; ++i)
        {
            if (predicate(this[i]))
                return i;
        }
    ...
    public readonly void Sort(Comparison<T> comparison) =>
        _items.AsSpan()[..Count].Sort(comparison);
    ...
    public readonly Span<T> AsSpan() => new(_items, 0, Count);
    ...
    /// <summary>
    /// Converts this inline list into a read-only memory.
    /// </summary>
    /// <returns>The read-only memory.</returns>
    public readonly ReadOnlyMemory<T> AsReadOnlyMemory() => new(_items, 0, Count);
    ...
    public readonly string ToString(Func<T, string> formatter) =>
...
static class InlineList
{
        ...
        StringBuilder target)
    {
        ...
        {
            target.Append(formatter(span[i]));
            if (i + 1 < e)
                target.Append(", ");
        }
    }
        ...
        Func<T, string> formatter)
    {
        ...
        {
            string result = formatter(span[0]);
            if (span.Length > 1)
                result += ", " + formatter(span[1]);
            if (span.Length > 2)
                result += ", " + formatter(span[2]);
            return result;
        }
        else
        {
            ...
            for (int i = 0, e = span.Length; i < e; ++i)
            {
                result.Append(formatter(span[i]));
                if (i + 1 < e)
                    result.Append(", ");
            }
