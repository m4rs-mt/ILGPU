...
internal static class MiniFloatSupport
{
    ...
    {
        byte[] shiftTable = new byte[512]; // For each possible 8-bit exponent value in the IEEE 754 format

        // Calculate the shift required to adjust a 23-bit IEEE 754 mantissa to the target mantissa size
        int standardShift = 23 - mantissaBitsInFloat8;

        for (int i = 0; i < 256; i++)
        {
            int exponent = i - 127; // Adjusting for the bias of 32-bit floats (IEEE 754 single-precision)

            if (exponent <= -127) // Handling special cases (subnormals, zeros)
            {
                // For IEEE 754, subnormals have an exponent of -126 (after adjusting for bias).
                // However, when the exponent bits are all zeros (which translates to -127 after bias adjustment),
                // it represents either zero or subnormal numbers.
                // In these cases, it might make sense to shift the mantissa fully, as these are too small to represent directly.
                shiftTable[i] = (byte)standardShift;
            }
            else if (exponent >= -126 && exponent <= 128) // Normal range
            {
                // The shift for normal numbers adjusts the mantissa to fit the mini float's smaller size.
                shiftTable[i] = (byte)standardShift;
            }
            else // Handling very large exponents (overflow cases)
            {
                // For very large exponent values that the mini float cannot represent,
                // it might be necessary to handle these as special cases, potentially clamping them.
                // However, without specific handling for infinity or NaN (not representable in all mini float formats),
    ...
    internal static byte[] GenerateBaseTable(int exponentBitsInFloat8)
    {
        ...
        int miniFloatBias = (1 << (exponentBitsInFloat8 - 1)) - 1; // Correctly calculate the mini-float bias
        int maxExponentValue = (1 << exponentBitsInFloat8) - 1; // Maximum value based on exponent bits
