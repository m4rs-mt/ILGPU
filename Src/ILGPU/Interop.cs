...
public static partial class Interop
{
    ...
    {
        long offset = XMath.Min(length, ComputeAlignmentOffset(ptr, alignmentInBytes));
        return ptr + offset;
    }
    ...
    public static long ComputeRelativeSizeOf<TFirst, TSecond>(long numSecondElements)
        ...
        ComputeRelativeSizeOf(SizeOf<TFirst>(), SizeOf<TSecond>(), numSecondElements);

    /// <summary>
    /// Computes number of elements of the first element size that are required to store
    /// a type described by second element size in unmanaged memory.
    /// </summary>
    /// <param name="firstElementSize">
    /// The element size of primary elements.
    /// </param>
    /// <param name="secondElementSize">
    /// The element size of secondary elements.
    /// </param>
    /// <param name="numSecondElements">
    /// The number of second-type elements to be stored.
    /// </param>
    /// <returns>
    /// The number of required of first-element instances to store
    /// <paramref name="numSecondElements"/>
    /// instances of the second-element instance.
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [NotInsideKernel]
    public static long ComputeRelativeSizeOf(
        int firstElementSize,
        int secondElementSize,
        long numSecondElements)
    {
        if (firstElementSize < 1)
            throw new ArgumentOutOfRangeException(nameof(firstElementSize));
        if (secondElementSize < 1)
            throw new ArgumentOutOfRangeException(nameof(secondElementSize));
        if (numSecondElements < 1)
            throw new ArgumentOutOfRangeException(nameof(numSecondElements));

        long relativeSize = XMath.DivRoundUp(
            secondElementSize * numSecondElements,
            firstElementSize);
        return relativeSize;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [InteropIntrinsic]
    public static Half IntAsFloat(ushort value) => new(value);
