...
public readonly partial record struct CLCVersion(int Major, int Minor)
{
    #region Static
    ...
    [GeneratedRegex("\\s*(CL|OpenCL C)?\\s*([0-9]+).([0-9]+)")]
    private static partial Regex OpenCLVersionRegex();

    /// <summary>
    /// The internal regex that is used to parse OpenCL C versions.
    /// </summary>
    private static readonly Regex VersionRegex = OpenCLVersionRegex();
