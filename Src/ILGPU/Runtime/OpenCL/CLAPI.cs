...
unsafe partial class CLAPI
{
    ...
    internal interface ILaunchHandler
    {
        ...
        static abstract CLError PreLaunchKernel(
            CLStream stream,
            CLKernel kernel,
    ...
    internal readonly struct DefaultLaunchHandler : ILaunchHandler
    {
        ...
        public static CLError PreLaunchKernel(
            CLStream stream,
            CLKernel kernel,
            in KernelConfig config) =>
            CLError.CL_SUCCESS;
            ...
            in KernelConfig config)
        {
            ...
            CLException.ThrowIfFailed(
                CurrentAPI.SetKernelArgumentUnsafeWithKernel(
                    kernel,
                    CLAccelerator.DynamicSharedMemoryParameterIndex,
                    config.SharedMemoryBytes,
                    null));
            // The length of the local buffer (in bytes).
            return CurrentAPI.SetKernelArgument(
                kernel.KernelPtr,
                CLAccelerator.DynamicSharedMemoryLengthParameterIndex,
                config.SharedMemoryBytes);
        }
    }
    ...
    public CLError BuildProgram(
        ...
        IntPtr device,
        string options) =>
        clBuildProgram(program, 1, &device, options, IntPtr.Zero, IntPtr.Zero);
    ...
    public CLError BuildProgram(
        ...
        int numDevices,
        string options) =>
        clBuildProgram(program, numDevices, devices, options, IntPtr.Zero, IntPtr.Zero);
        ...
        string options)
    {
        fixed (IntPtr* ptr = &devices[0])
        {
            return BuildProgram(program, ptr, devices.Length, options);
        }
    }
    ...
    public CLError CreateKernel(
        ...
        out IntPtr kernel)
    {
        kernel = clCreateKernel(program, kernelName, out CLError errorStatus);
        return errorStatus;
    }
    ...
    /// <returns>The error code.</returns>
    public CLError ReleaseKernel(IntPtr kernel) => clReleaseKernel(kernel);
    ...
    public CLError SetKernelArgumentUnsafe(
        ...
        int size,
        void* value) =>
        clSetKernelArg(kernel, index, new IntPtr(size), value);
    ...
    public CLError SetKernelArgumentUnsafeWithKernel(
        ...
        int size,
        void* value) =>
        clSetKernelArg(kernel.KernelPtr, index, new IntPtr(size), value);
        ...
        where THandler : struct, ILaunchHandler
    {
        ...
        var result = THandler.PreLaunchKernel(stream, kernel, kernelConfig);

        IntPtr* globalWorkSizes = stackalloc IntPtr[3];
        globalWorkSizes[0] = new IntPtr(kernelConfig.NumThreads);
        globalWorkSizes[1] = new IntPtr(1);
        globalWorkSizes[2] = new IntPtr(1);

        IntPtr* localWorkSizes = stackalloc IntPtr[3];
        localWorkSizes[0] = new IntPtr(kernelConfig.GroupSize);
        localWorkSizes[1] = new IntPtr(1);
        localWorkSizes[2] = new IntPtr(1);
    ...
    /// <returns>The error code.</returns>
    internal CLError ReleaseEvent(IntPtr @event) => clReleaseEvent(@event);
    ...
    internal CLError GetEventInfo(
        ...
        void* param_value) =>
        clGetEventInfo(@event, param_name, param_value_size, param_value, IntPtr.Zero);
