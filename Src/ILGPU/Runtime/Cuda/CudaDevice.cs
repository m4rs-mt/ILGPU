...
/// </summary>
public sealed class CudaDevice : Device, IDeviceAcceleratorTypeInfo
{
    #region Static
    ...
    internal CudaDevice(int deviceId) : base(AcceleratorType.Cuda)
    {
        if (deviceId < 0)
    ...
    private void InitGridInfo()
    {
        ...
        // Determine maximum grid size in linear space and setup optimal kernel size
        int maxGridSize = CurrentAPI.GetDeviceAttribute(
                DeviceAttributeKind.CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_X, DeviceId);
        OptimalKernelSize = new(maxGridSize, MaxNumThreadsPerMultiprocessor / 2);
    }
