...
public struct XorShift32 : IEquatable<XorShift32>, IRandomProvider<XorShift32>
{
    ...
    public XorShift32 NextProvider() => new(NextUInt());

    /// <inheritdoc cref="IRandomProvider{TProvider}.CreateProvider(System.Random)"/>
    public static XorShift32 CreateProvider(System.Random random) =>
        Create(random);
    ...
    public static XorShift32 CreateProvider<TProvider>(ref TProvider random)
        where TProvider : struct, IRandomProvider<TProvider> =>
        new((uint)random.Next() + 1U);
