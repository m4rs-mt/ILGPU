...
using static ILGPU.Random.RandomExtensions;
    ...
    IRandomProvider<XorShift64Star>
{
    ...
    public XorShift64Star NextProvider() => new(NextULong());

    /// <inheritdoc cref="IRandomProvider{TProvider}.CreateProvider(System.Random)"/>
    public static XorShift64Star CreateProvider(System.Random random) =>
        Create(random);
    ...
    public static XorShift64Star CreateProvider<TRandomProvider>(
        ref TRandomProvider random)
        where TRandomProvider : struct, IRandomProvider<TRandomProvider> =>
        new((ulong)random.NextLong() + 1UL);
